...
class HMM:
    ...
    def backward(self, Osequence):
        ...
        for t in range(L - 2, -1, -1):
            for s in range(S):
                beta[s, t] = np.dot(np.multiply(self.A[s, :], self.B[:, O[t + 1]]), beta[:, t + 1])
        return beta
    ...
    def sequence_prob(self, Osequence):
        ...
        prob = np.sum(np.multiply(alpha[:, -1], beta[:, -1]))
        return prob
    ...
    def likelihood_prob(self, Osequence):
        ...
        for t in range(L - 1):
            for s in range(S):
                for s_ in range(S):
                    prob[s, s_, t] = alpha[s, t] * self.A[s, s_] * beta[s_, t + 1] * self.B[s_, O[t + 1]] / seq_prob
        return prob
    ...
    def viterbi(self, Osequence):
        ...
        O = self.find_item(Osequence)
        delta = np.zeros([S, L])
        diff = np.zeros([S, L], dtype=int)

        delta[:, 0] = np.multiply(self.pi, self.B[:, O[0]])

        for t in range(1, L):
            for s in range(S):
                aXdelta = np.multiply(self.A[:, s], delta[:, t - 1])
                delta[s, t] = self.B[s, O[t]] * np.max(aXdelta)
                diff[s, t] = np.argmax(aXdelta)

        ind = np.argmax(delta[:, L - 1])
        path.append(self.find_key(self.state_dict, ind))
        for t in range(L - 1, 0, -1):
            zt_1 = diff[ind, t]
            ind = zt_1
            path.insert(0, self.find_key(self.state_dict, zt_1))
        return path
