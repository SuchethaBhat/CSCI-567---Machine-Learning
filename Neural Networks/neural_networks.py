...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        forward_output = X * np.where(X > 0, True, False)
        return forward_output
    ...
    def backward(self, X, grad):
        ...
        backward_output[np.where(X <= 0, True, False)] = 0
        return backward_output
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha <= 0.0:
                    ...
                    ###################################################################################################
                    momentum[str(module_name + "_" + key)] = momentum[str(
                        module_name + "_" + key)] * _alpha - _learning_rate * g
